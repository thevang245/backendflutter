...
PostSchema.methods.updateRating = function () {
  ...
  // Cập nhật tổng điểm rating trung bình, làm tròn 2 chữ số sau dấu chấm
  this.totalRating = parseFloat((total / validComments.length).toFixed(2));
...
app.post('/register', async (req, res) => {
  const { avatar, name, phone, email, password } = req.body;
  ...
  try {
    ...
    const newUser = new User({ avatar, name, phone, email, password: hashedPassword });
    await newUser.save();
...
app.post('/facebook-login', async (req, res) => {
  ...
  if (!uid || !email) return res.status(400).json({ error: 'Thiếu uid hoặc email.' });
  ...
  try {
    ...
    if (!user) {
      ...
      user = new User({
        name,
        email,
        firebaseUid: uid, // Lưu firebaseUid để phân biệt Google/Facebook đều qua Firebase
        role,
      });
      await user.save();
    }
...
app.post('/google-login', async (req, res) => {
  const { uid, email, name, phone,avatar, role, fcmToken } = req.body; // Nhận thêm fcmToken từ client
  ...
  try {
    ...
    if (!user) {
      // Tạo mới tài khoản
      user = new User({ avatar, name, email, googleUid: uid, role, fcmToken });
    } else {
      // Cập nhật token FCM nếu có
    ...
    res.status(200).json({
      ...
      token,
      user: { id: user._id, avatar: user.avatar, name: user.name,phone: user.phone, email: user.email, role: user.role, fcmToken: user.fcmToken },
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Lỗi server.' });
...
app.post('/update-avatar', async (req, res) => {
  ...
  // Kiểm tra đầu vào
  if (!userId || !avatar) {
    return res.status(400).json({ error: 'Thiếu userId hoặc avatar' });
  }

  try {
    // avatar giờ là URL, không cần up tiếp Cloudinary
    const updatedUser = await User.findByIdAndUpdate(
      userId,
      { avatar: avatar }, // Cập nhật URL
      { new: true }
    );
    ...
    res.status(200).json({
      ...
      avatarUrl: avatar, // Trả về URL mới
      user: updatedUser,
    });
  } catch (err) {
    console.error(err);
...
app.get('/posts/:id', async (req, res) => {
  ...
  try {
    ...
    // 🔥 Lấy thông tin chủ bài đăng từ bảng User
    const user = await User.findById(post.userId, 'name phone'); // Chỉ lấy name, phone
    if (user) {
      post.contactName = user.name;
      post.contactPhone = user.phone;
    }

    post.updateRating();
    await post.save(); // Lưu lại thay đổi

    // Trả về bài đăng với thông tin cập nhật
...
app.get('/user/:userId', async (req, res) => {
  ...
  try {
    // Tìm user theo ID
    const user = await User.findById(userId).select('-password -resetOtp -otpExpires -fcmToken'); // Ẩn các trường nhạy cảm
    ...
    res.status(500).json({ error: 'Đã xảy ra lỗi khi truy vấn thông tin người dùng' });
  }
});
...
app.post('/update-profile/:userId', async (req, res) => {
  ...
  console.log('Received data:', { userId, name, phone, email });
  ...
  try {
    const objectId = new mongoose.Types.ObjectId(userId); // Chuyển userId sang ObjectId

    // Kiểm tra số điện thoại đã tồn tại chưa (ngoại trừ user hiện tại)
    const existingPhone = await User.findOne({ phone: phone, _id: { $ne: objectId } });
    if (existingPhone) {
    ...
    const existingEmail = await User.findOne({ email: email, _id: { $ne: objectId } });
    if (existingEmail) {
...
// API thiết lập mật khẩu mới (chỉ khi chưa có mật khẩu)
app.post('/set-password', async (req, res) => {
  ...
  try {
    ...
    // Nếu đã có mật khẩu thì không cho set lại (bắt buộc phải dùng "đổi mật khẩu")
    if (user.password) {
      return res.status(400).json({ msg: 'Tài khoản đã có mật khẩu, vui lòng sử dụng chức năng đổi mật khẩu' });
    }
    ...
    res.status(200).json({ msg: 'Thiết lập mật khẩu thành công' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ msg: 'Có lỗi xảy ra khi thiết lập mật khẩu' });
  }
});
...
app.get('/check-password-field/:userId', async (req, res) => {
  const { userId } = req.params;

  try {
    // Tìm user theo ID và kiểm tra trường password có tồn tại không
    const user = await User.findOne(
      { _id: userId, password: { $exists: true } },
      { _id: 1 } // Chỉ cần trả về _id, không cần dữ liệu khác
    );

    const hasPasswordField = !!user; // Nếu tìm thấy user => có trường password

    res.status(200).json({ hasPasswordField });
  } catch (err) {
    console.error(err);
    res.status(500).json({ msg: 'Có lỗi xảy ra khi kiểm tra trường password' });
  }
});
...
app.get('/bookings/user/:userId', async (req, res) => {
  ...
  try {
    // 🔥 Lấy tất cả các booking của chủ trọ (userId là chủ phòng)
    const bookings = await Booking.find({ userId: userId }).sort({ createdAt: -1 });

    if (bookings.length === 0) {
      return res.status(200).send([]); // Không có booking nào
    }

    // 🔥 Lấy danh sách ID của người đặt lịch (requestuserId) từ các booking
    const requestUserIds = [...new Set(bookings.map(booking => booking.requestuserId))]; // Loại bỏ trùng lặp

    // 🔥 Truy vấn tất cả thông tin user một lần để tối ưu performance
    const users = await User.find({ _id: { $in: requestUserIds } }, 'name phone');

    // Chuyển danh sách user thành Map để dễ dàng truy xuất
    const userMap = new Map(users.map(user => [user._id.toString(), user]));

    // 🔥 Cập nhật từng booking với thông tin người đặt lịch mới nhất
    const updatedBookings = bookings.map(booking => {
      const user = userMap.get(booking.requestuserId.toString());
      return {
        ...booking.toObject(),
...
app.post('/posts/:postId/comment', async (req, res) => {
  const { userId, bookingId, name, comment,reply, rate, createdAt, parentId } = req.body;
  const postId = req.params.postId;
  console.log('thong tin nhan duoc tu client')
...
app.get('/comments/:postId', async (req, res) => {
  try {
    ...
    // 🔥 Tìm bài viết và lấy danh sách bình luận
    ...
    // 🔥 Lấy danh sách userId từ comments (chỉ lấy ID duy nhất)
    const userIds = [...new Set(post.comments.map(comment => comment.userId))];

    // 🔥 Truy vấn thông tin người dùng theo userId (lấy cả name & avatar)
    const users = await User.find({ _id: { $in: userIds } }, 'name avatar').lean();
    const userMap = users.reduce((map, user) => {
      map[user._id] = { name: user.name, avatar: user.avatar }; // Map userId -> {name, avatar}
      return map;
    }, {});
    ...
    const updatedComments = post.comments.map(comment => ({
      ...comment,
      userName: userMap[comment.userId]?.name || 'Người dùng ẩn danh', 
      userAvatar: userMap[comment.userId]?.avatar || null // Nếu không có avatar thì trả về null
    }));
