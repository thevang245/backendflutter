...
app.post('/booking', async (req, res) => {
  ...
  try {
    ...
    await booking.save();
    const landlord = await User.findById(userId); // Tìm chủ trọ bằng userId
    try {
      if (landlord && landlord.fcmToken) {
        const message = {
          ...
          data: {
           role: "owner"  // hoặc "owner"
          },
        };

        console.log("📨 Đang gửi thông báo:", message);

        const response = await admin.messaging().send(message);
        console.log("✅ Thông báo đã gửi thành công:", response);
      } else {
        console.log("⚠️ Chủ trọ không có token FCM.");
      }
    } catch (error) {
...
app.post('/bookings/:bookingId/viewed', async (req, res) => {
  ...
  try {
    // 1️⃣ Cập nhật trạng thái đặt phòng thành "viewed"
    ...
    // 2️⃣ Lấy thông tin người thuê để gửi thông báo
    const tenant = await User.findById(booking.requestuserId); // Người thuê
    if (tenant && tenant.fcmToken) {
      const message = {
        ...
        data: {
          action: 'review_request', // Dữ liệu để điều hướng trong Flutter
          bookingId: bookingId,
        },
      };
...
app.post('/bookings/:bookingId/approve', async (req, res) => {
  ...
  try {
    // 1️⃣ Cập nhật trạng thái thành "approved"
    const booking = await Booking.findByIdAndUpdate(
      bookingId,
      { status: 'approved' },
      { new: true }
    ...
    // 2️⃣ Lấy thông tin người thuê để gửi thông báo
    const tenant = await User.findById(booking.requestuserId); // requestuserId là người thuê
    if (!tenant || !tenant.fcmToken) {
      return res.status(400).send({ message: 'Renter has no FCM token' });
    }

    // 3️⃣ Gửi thông báo Firebase
    const message = {
      token: tenant.fcmToken,
      notification: {
      ...
      data: {
        action: 'booking_approved', // Điều hướng trong app
        bookingId: bookingId,
      },
    };
...
app.post('/bookings/:bookingId/reject', async (req, res) => {
  ...
  try {
    // 1️⃣ Cập nhật trạng thái thành "rejected"
    const booking = await Booking.findByIdAndUpdate(
      bookingId,
      { status: 'rejected' },
      { new: true }
    ...
    // 2️⃣ Lấy thông tin người thuê từ database
    const tenant = await User.findById(booking.requestuserId); // ✅ Sửa lại key cho đúng
    if (!tenant || !tenant.fcmToken) {
      return res.status(400).send({ message: 'Renter has no FCM token' });
    }
    ...
    const message = {
      ...
      data: {
        action: 'booking_rejected', // Điều hướng trong app
        bookingId: bookingId,
      },
    };
